        ...
        for (int j = 0; j < player.Inventory.Length; j++)
        {
            ...
            switch (GameManager.ItemTable[player.Inventory[j].id].ItemType)
            {
                ...
                case ItemType.Accessories:
                    ...
                    texts[j] = texts[j] + ", 스탯1 : (" + ((AccessoriesUniqueData)player.Inventory[j].uniqueData).upgradeStat[0].UpgradeStatType+", "+ ((AccessoriesUniqueData)player.Inventory[j].uniqueData).upgradeStat[0].UpgradeStatType;
                    texts[j] = texts[j] + ", 스탯2 : (" + ((AccessoriesUniqueData)player.Inventory[j].uniqueData).upgradeStat[1].UpgradeStatType+", "+ ((AccessoriesUniqueData)player.Inventory[j].uniqueData).upgradeStat[1].UpgradeStatType;
                    break;
